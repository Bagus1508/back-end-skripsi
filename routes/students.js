      ...
      include: [
        {
          ...
          attributes: [
            ...
            'number_id',
            'classes',
            [Sequelize.literal('(SELECT name FROM class_rooms WHERE class_rooms.id = userDetails.classes LIMIT 1)'), 'classes_desc'],
            'subject_id',
            'email',
            'gender',
            'birth_date',
...
router.put('/update/:id', async function(req, res, next) {
  try {
    ...
    res.status(201).json({
      status: 'OK',
      messages: 'Data Siswa berhasil diubah',
      data: updatedUser
    });
  } catch (error) {
    console.error('Error:', error.message); // Debug log
...
router.delete('/delete/:id', async function(req, res, next) {
  try {
    ...
    res.json({
      status: 'OK',
      messages: 'Data Siswa berhasil dihapus'
    });
  } catch (err) {
    res.status(500).json({
      status: 'ERROR',
